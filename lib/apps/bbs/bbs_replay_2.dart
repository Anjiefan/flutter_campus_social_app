...
class Reply2Page<T extends Model> extends StatefulWidget {
  int index;
  bool comment;
  Reply2Page(
      {
  ...
  @override
  ReplyPageState createState() => new ReplyPageState<T>(index: index,comment: comment);
}

class ReplyPageState<T extends Model> extends State<Reply2Page> {
  ...
  Widget build(BuildContext context) {
    ...
    _commentdModel= ScopedModel.of<BaseComment>(context,rebuildOnChange: true);
    List<Widget> widgets=[
      BBSDetailCommentCard<T>(index: index,),
      new Container(
        child: Row(
          children: <Widget>[
            Expanded(
        ...
        child: new EasyRefresh(
          ...
          onRefresh: () async {
            NetUtil.get(Api.COMMENTS, (data) async{
              ...
              params: {'status_id':_commentdModel.commentList[index].comment.status.objectId,
                'comment_id':_commentdModel.commentList[index].comment.objectId,'page':0},
          ...
          loadMore: () async {
            NetUtil.get(Api.COMMENTS, (data) async{
              ...
              params: {'status_id':_commentdModel.commentList[index].comment.status.objectId,
                'comment_id':_commentdModel.commentList[index].comment.objectId,'page':page},
              headers: {"Authorization": "Token ${_userAuthModel.session_token}"},
            );
          },
        ),
...
//                height: 30,
  ...
  void _handle_send_comment(){
    ...
    NetUtil.post(Api.COMMENTS, (data) {
      ...
      NetUtil.get(Api.COMMENTS+id+'/', (data) {
        ...
        NetUtil.get(Api.COMMENTS+_commentdModel.commentList[index].comment.objectId+'/', (data) async{
      ...
      //TODO 界面显示数据
        ...
        params: {"text": _controller.text
          , "status": _commentdModel.commentList[index].comment.status.objectId
          ,"comments":_commentdModel.commentList[index].comment.objectId
    });
  ...
  void _handle_comments(){
    NetUtil.get(Api.COMMENTS, (data) async{
      ...
      params: {'status_id':_commentdModel.commentList[index].comment.status.objectId,
        'comment_id':_commentdModel.commentList[index].comment.objectId},
      headers: {"Authorization": "Token ${_userAuthModel.session_token}"},
    );
  }
}
